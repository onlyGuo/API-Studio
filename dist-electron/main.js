...
function mimeForFile(file) {
    ...
    return { '.json': 'application/json', '.txt': 'text/plain', '.csv': 'text/csv', '.xml': 'application/xml', '.pdf': 'application/pdf', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp', '.zip': 'application/zip' }[extension] || 'application/octet-stream';
}
...
app.whenReady().then(() => {
    ...
    ipcMain.handle('store:load', async () => {
        ...
        for (const file of [current, ...legacy]) {
            try {
    ...
    ipcMain.handle('project:import', async () => {
        ...
        for (const file of result.filePaths)
            projects.push(JSON.parse(await fs.readFile(file, 'utf8')));
        return projects;
    });
    ...
    ipcMain.on('http:stream:start', (event, payload) => {
        ...
        if (input.timeout && input.timeout > 0)
            timer = setTimeout(() => controller.abort(), input.timeout);
        const send = (message) => {
            if (!event.sender.isDestroyed())
                event.sender.send('http:stream:event', { id, message });
        };
        void (async () => {
            try {
                if (input.validateSSL === false)
                ...
                if (reader) {
                    while (true) {
                        ...
                        if (chunk)
                            send({ type: 'chunk', chunk, bytes: value.byteLength });
                    }
                    const tail = decoder.decode();
                    if (tail)
                        send({ type: 'chunk', chunk: tail, bytes: 0 });
                }
    ...
    ipcMain.on('http:stream:cancel', (_event, id) => {
        streamControllers.get(id)?.abort();
        streamControllers.delete(id);
    });
    ipcMain.on('theme:set', (_event, value) => { nativeTheme.themeSource = value; });
    createWindow();
